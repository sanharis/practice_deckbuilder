...
@onready var blinkmotherfucker: AnimationPlayer = %BLINKMOTHERFUCKER
...
func _blink_timer_setup() -> void:
	timer.wait_time = randf_range(0.5, 8.0)
	timer.start()

func _generate_shop_cards() -> void:
	var shop_card_array : Array[Card] = []
	var available_cards := char_stats.draft_cards.cards.duplicate(true)
...
func _generate_shop_relics() -> void:
	...
	var available_relics := shop_relics.filter(
		func(relic:Relic):
			...
			var already_owned := relic_handler.has_relic(relic.id)
			return can_appear and not already_owned
	)
