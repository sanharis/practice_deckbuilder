...
func add_relics(relic_array: Array[Relic]) -> void:
	for relic: Relic in relic_array:
		add_relic(relic)
...
func has_relic(id: String) -> bool:
	for relic_ui: RelicUI in relics.get_children():
		if is_instance_valid(relic_ui.relic) and relic_ui.relic.id == id:
			return true
